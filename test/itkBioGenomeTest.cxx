...
itkBioGenomeTest(int, char *[])
{
  ...
  if (itk::Math::abs(genome.GetExpressionLevel("Tubulin") - genome2.GetExpressionLevel("Tubulin")) > tolerance)
  {
    std::cerr << "Error in Copy()" << std::endl;
    return EXIT_FAILURE;
  }

  if (itk::Math::abs(genome.GetExpressionLevel("Cyclin") - genome2.GetExpressionLevel("Cyclin")) > tolerance)
  {
    std::cerr << "Error in Copy()" << std::endl;
    return EXIT_FAILURE;
  }
